    ...
    try {
      ...
      const result = await res.json();
        ...
        const sortedCouples = [...couplesData].sort((a, b) => {
          ...
          const nameA = a.athlete1 ? `${a.athlete1.last_name} ${a.athlete1.first_name}`.toLowerCase() : "";
          const nameB = b.athlete1 ? `${b.athlete1.last_name} ${b.athlete1.first_name}`.toLowerCase() : "";
          return nameA.localeCompare(nameB);
        });
        setCouples(sortedCouples);
        if (sortedCouples.length === 1) selectCouple(sortedCouples[0]);
                       ...
                       <div className="flex items-center gap-4">
                         ...
                         <div>
                           ...
                           <p className="font-black text-xl">
                             {`${selectedCouple.athlete1?.first_name || ''} ${selectedCouple.athlete1?.last_name || ''}${selectedCouple.athlete2 ? ` & ${selectedCouple.athlete2.first_name} ${selectedCouple.athlete2.last_name}` : ''}`.trim()}
                           </p>
                         </div>
                       </div>
                    </div>
