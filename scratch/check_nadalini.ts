...
async function checkNadaliniVenturini() {
  ...
  if (linkErr) {
    ...
    // Find matching links for Caterina
  ...
  const { data: couples, error: cplErr } = await supabase
    ...
    .or(`athlete1_id.in.(${athleteIds.map(id => `'${id}'`).join(',')}),athlete2_id.in.(${athleteIds.map(id => `'${id}'`).join(',')})`);
  ...
  for (const couple of couples) {
    ...
    // Rule 2 check: Athlete 2 linked to Caterina
    ...
    // Rule 3 check: Caterina's name matches any of couple's responsabili
    // In SQL we use match_names, let's simulate it or check if "Caterina Alberghini" is in couple.responsabili
    const hasNameInResponsabili = (couple.responsabili || []).some((resp: string) => {
      // Basic match
