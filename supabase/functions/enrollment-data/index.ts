...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "search-athletes") {
      ...
      const { data, error } = await supabase.from("athletes").select("id, code, first_name, last_name, category, class, qr_code, discipline_info").eq("is_deleted", false).or(`first_name.ilike.${pattern},last_name.ilike.${pattern},code.ilike.${pattern},qr_code.ilike.${pattern}`).limit(20);
