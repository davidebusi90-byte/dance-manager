...
Deno.serve(async (req) => {
  ...
  console.log(`[${requestId}] import-competitors: Sync process started`);
  ...
  try {
    ...
    try {
      ...
      console.log(`[${requestId}] import-competitors: Payload logged to ${filename}`);
    } catch (e) {
    ...
    if (missingCodes.length > 0) {
      console.log(`[${requestId}] import-competitors: Soft-deleting ${missingCodes.length} missing athletes`);
    ...
    if (couplesToUpsert.length > 0) {
      // Deactivate couples not in the new payload
    ...
    console.log(`[${requestId}] import-competitors: Process completed successful=${results.successful} failed=${results.failed}`);
    ...
    console.error(`[${requestId}] import-competitors: Global error:`, error);
